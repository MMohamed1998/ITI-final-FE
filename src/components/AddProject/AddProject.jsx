    ...
    <div className={`modal-content ${styles.add_Project}`}>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        <div className="form-group">
          ...
          {errors.title && <span className="text-danger ">Title must be at least 25 characters long</span>}
        </div>
        <div className="form-group">
          <textarea   {...register("description",{ required: true, minLength: 50 })} className={ ` form-control my-3 ${styles.textArea}`} placeholder="Description" />
          {errors.description && <span className="text-danger ">description must be at least 50 characters long</span>}
        ...
        <div className="form-group">
          ...
          {errors.expectedPrice && <span className="text-danger">Please select an expected price range</span>}
        </div>
        <div className="form-group">
          <input {...register("expectedTime", { required: true, pattern: /^[0-9]+$/ })} className="form-control p-2 my-3" type="number" placeholder="Expected Time" />
          {errors.expectedTime && <span className="text-danger">Expected Time is required and must be a number</span>}
        </div>
        <div className="form-group">
        ...
        <div className="form-group d-flex flex-wrap my-3">
          {skills.map((skill, index) => (
            <div key={index} className="form-check ">
              ...
              />
              <label className="form-check-label me-2" htmlFor={`checkbox-${index}`}>{skill}</label>
            </div>
          ))}
        </div>
        <button type="submit" style={{ backgroundColor: "#c59a5a",border:"unset"  }} className={` rounded-5 mb-3 text-white p-1 w-100`} disabled={isLoading} >
